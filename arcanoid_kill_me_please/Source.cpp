...
const float BLOKS_DISTANCE_X = (float)abs((WINDOW_WEIDH - (BLOCKS_IN_ROW * BLOCK_SIDE)) / (BLOCKS_IN_ROW + 1));
const float BLOKS_DISTANCE_Y = (float)abs((WINDOW_HEIGHT - WINDOW_HEIGHT/4 - BLOCK_SIDE - BLOCK_SIDE * BLOCKS_IN_COL) / BLOCKS_IN_COL + 1);
